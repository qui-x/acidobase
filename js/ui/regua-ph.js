...
SIAB.regua = (() => {
  ...
  // concentracao: acrescenta a linha de [H₃O⁺] em potências de 10 (escala logarítmica).
  const SOBRE = ['⁰', '¹', '²', '³', '⁴', '⁵', '⁶', '⁷', '⁸', '⁹'];
  const potencia = n => `10${n ? '⁻' : ''}${String(n).split('').map(d => SOBRE[d]).join('')}`;
  function svg(pH, { neutro = 7, rotulo = 'Régua de pH', faixa = null, concentracao = false } = {}) {
    ...
    const yRotulo = faixa ? 50 : 44;
    const linhaH = concentracao ? `<text class="regua-conc-titulo" x="8" y="${yRotulo + 14}">[H₃O⁺] mol/L</text>${[0, 7, 14].map(n => `<text class="regua-conc" x="${x(n)}" y="${yRotulo + 26}" text-anchor="${n === 0 ? 'start' : n === 14 ? 'end' : 'middle'}">${potencia(n)}</text>`).join('')}` : '';
    const textoConc = concentracao ? ' Cada unidade de pH corresponde a 10 vezes mais ou menos H₃O⁺.' : '';
    return `<svg class="regua-svg" viewBox="0 0 240 ${yRotulo + 4 + (concentracao ? 26 : 0)}" role="img" aria-label="${descricao}${textoConc}">
      <defs>${gradiente(id)}</defs>
      <rect x="8" y="16" width="224" height="14" rx="7" fill="url(#${id})"/>
      ${neutroMarca}${marcador}${viragem}
      <text x="8" y="${yRotulo}">0</text><text x="${x(7)}" y="${yRotulo}" text-anchor="middle">7</text><text x="232" y="${yRotulo}" text-anchor="end">14</text>
      ${linhaH}
    </svg>`;
  }
  return { svg };
